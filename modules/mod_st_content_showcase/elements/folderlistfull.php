	...
	protected function getInput()
	{
		...
		$doc->addScriptDeclaration('
			window.addEvent("domready", function(){
				...
				wrapper.inject($("jform_params_ifolder_folders"), "after");
				...
				var myRequest = new Request({
				    ...
				    onSuccess: function(responseText){
						...
						if (oImages) 
						{
							oImages.ifolder_image.each(function(el, index){
									if (el != "") 
									{
										el =  el.replace("\\\", "/");
										html += "<li><img style=\"width: 100%;\" src=\"'.JURI::root().'"+ el +"\"/>";	
										html += "<div class=\"info\">";
										html += "<strong>Title</strong><br/><input name=\"jform[params][ifolder_image][]\" type=\"hidden\" value=\""+ el +"\"/>";
										html += "<input name=\"jform[params][ifolder_ititle][]\" type=\"text\" value=\""+ oImages.ifolder_ititle[index] +"\"/>";
										html += "<strong>Introtext</strong><br/><textarea cols=\"30px\" rows=\"5\" name=\"jform[params][ifolder_iintrotext][]\">" + oImages.ifolder_iintrotext[index] + "</textarea>";
										html += "</div>";
				...
				myRequest.send();
					setTimeout(function(){
						$("jform_params_ifolder_folders").setStyle("display", "block");
						$("jform_params_ifolder_folders_chzn").setStyle("display", "none");
					}, 1000);
					$("jform_params_ifolder_folders").addEvent("mouseup", function(event){
						event.preventDefault();
						var moduleParams = savedData.params,
						folders = $("jform_params_ifolder_folders").getSelected(),
						string = "";
						    ...
						    onSuccess: function(responseText){
								...
								Object.each(oImages, function(value, key){
									...
									value.each(function(el){
										try 
										{
											var imagePath = (key + "/" + el).replace("\\\", "/");
											var imageUrl = "'.JURI::root().'" + imagePath;
											console.log(moduleParams, moduleParams.ifolder_image.indexOf(imagePath), imagePath);
											if (!moduleParams || moduleParams.ifolder_image.indexOf(imagePath) < 0) 
											{
												
												html += "<li><img style=\"width: 100%;\" src=\" "+ imageUrl +" \" />";	
												html += "<div class=\"info\"><strong>Title</strong><br/><input disabled name=\"jform[params][ifolder_image][]\" type=\"hidden\" value=\""+ imagePath +"\">";
												html += "<input disabled name=\"jform[params][ifolder_ititle][]\" type=\"text\" value=\""+ el +"\">";
												html += "<strong>Introtext</strong><br/><textarea disabled cols=\"30px\" rows=\"5\" name=\"jform[params][ifolder_iintrotext][]\"></textarea>";
												html += "</div>";
												html += "<div class=\"btn-group st-button-group\">";
								...
								$$("#wrap-image-source .list-image-loaded li .btn-group > *").addEvent("click", function(){
									...
									if (el.hasClass("edit")) {
										...
										contain.getElements("input, textarea").removeProperty("disabled");
										moduleParams.ifolder_image.push(contain.getElement("input[name=\"jform[params][ifolder_image][]\"]").get("value"));
									} else if (el.hasClass("remove")) {
										contain.toggleClass("removed");
										if (contain.hasClass("removed")) {
											contain.getElements("input, textarea").setProperty("disabled", "disabled");
