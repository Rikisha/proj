...
class DFA{
	...
	public function hilight($str, 
	   ...
	   $encoding=null){
        ...
        $this->result = "";
		for($i = 0 ; $i < ($this->encoding?mb_strlen($str,$this->encoding):strlen($str)) ; $i++){
			$char = $this->encoding?mb_substr($str, $i, 1,$this->encoding):substr($str, $i, 1);
			$this->nextState($char);
		}
		return $this->result;
	}
    ...
    private function performReplace($content){
    	...
    	foreach($this->keywords as $keyword){
    	   $upper = $this->encoding?mb_strtoupper($content, $this->encoding):strtoupper($content);
    	   $upperKeyword = $this->encoding?mb_strtoupper($keyword, $this->encoding):strtoupper($keyword);
    	   ...
    	   while(( $currentPosition = $this->encoding?@mb_strpos($upper, $upperKeyword, $currentPosition, $this->encoding):
    	    ...
    	    $searchPositions[] = $currentPosition;
    	    $currentPosition+=$this->encoding?mb_strlen($upperKeyword, $this->encoding):strlen($upperKeyword);
    	   }
    ...
    private function endTagName(){
        $this->tagName = $this->encoding?mb_substr($this->buffer, $this->tagNameStartPosition, $this->currentPos - $this->tagNameStartPosition):
