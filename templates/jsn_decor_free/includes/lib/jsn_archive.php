 ...
 * @version $Id: jsn_archive.php 12119 2012-04-12 04:49:37Z giangnd $
 */

defined('_JEXEC') or die( 'Restricted access' );
class JSNArchive
{
	function JSNArchive($name)
	{
		$this->options = array (
			'basedir' => ".",
			'name' => $name,
	...
	public static function getInstance()
	{
		...
		{
			$instanceArchive = new JSNArchive();
		}
	...
	function createArchive()
	{
		...
		if ($this->options['inmemory'] == 0)
		{
			...
			else if ($this->JSNArchive = @fopen($this->options['name'] . ($this->options['type'] == "gzip" || $this->options['type'] == "bzip" ? ".tmp" : ""), "wb+"))
				chdir($pwd);
			else
			{
	...
	{
		if ($this->options['inmemory'] == 0)
			fwrite($this->JSNArchive, $data);
		else
			$this->JSNArchive .= $data;
	}
	...
	function listFiles($list)
	{
		...
		usort($files, array ("JSNArchive", "sortFiles"));
	...
	function downloadFile()
	{
		if ($this->options['inmemory'] == 0)
		{
			$this->error[] = "Can only use downloadFile() if JSNArchive is in memory. Redirect to file otherwise, it is faster.";
			return;
		}
		...
		$header .= "\"";
		header($header);
		header("Content-Length: " . strlen($this->JSNArchive));
		header("Content-Transfer-Encoding: binary");
		header("Cache-Control: no-cache, must-revalidate, max-age=60");
		header("Expires: Sat, 01 Jan 2000 12:00:00 GMT");
		print($this->JSNArchive);
	}

	function writeArchiveFile()
	{
		if ($this->options['name'] && $this->JSNArchive)
		{
			if (JFile::write($this->options['name'], $this->JSNArchive)) {
				return true;
			}
		}
		return false;
...
class JSNTarFile extends JSNArchive
{
	function JSNTarFile($name)
	{
		$this->JSNArchive($name);
	...
	function createTar()
	{
		...
		foreach ($this->files as $current)
		{
			...
			if (strlen($current['name2']) > 99)
			{
				...
				{
					$this->error[] = "Could not add {$path}{$current['name2']} to JSNArchive because the filename is too long.";
					continue;
				}
			}
	...
	function open_JSNArchive()
	{
		return @fopen($this->options['name'], "rb");
	}
}

class JSNTPLGZIPFile extends JSNTarFile
{
	function JSNTPLGZIPFile($name)
	{
		$this->JSNTarFile($name);
	...
	function create_gzip()
	{
		if ($this->options['inmemory'] == 0)
		{
			...
			{
				fseek($this->JSNArchive, 0);
				while ($temp = fread($this->JSNArchive, 1048576))
					gzwrite($fp, $temp);
				gzclose($fp);
				chdir($pwd);
			}
			...
			$this->JSNArchive = gzencode($this->JSNArchive);
...
class JSNBZIPFile extends JSNTarFile
{
	function JSNBZIPFile($name)
	{
		$this->JSNTarFile($name);
	...
	function createBzip()
	{
		if ($this->options['inmemory'] == 0)
		{
			...
			$this->JSNArchive = bzcompress($this->JSNArchive, $this->options['level']);
...
class JSNZIPFile extends JSNArchive
{
	function JSNZIPFile($name)
	{
		$this->JSNArchive($name);
