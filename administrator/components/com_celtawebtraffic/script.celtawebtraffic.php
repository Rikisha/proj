...
class Com_CeltawebtrafficInstallerScript
{
	/** @var string The component's name */
	protected $_coalaweb_extension = 'com_celtawebtraffic';
	...
	private $installation_queue = array(
		...
		'modules' => array(
			...
			'site' => array(
				'celtawebtraffic' => array('left', 0),
			)
		),
		// plugins => { (folder) => { (element) => (published) }* }*
		'plugins' => array(
			'celtawebtraffic' => array(
			),
			'content' => array(
			),
			'system' => array(
...
//		$this->_killUpdateSite();
	...
	{
		if(!count($this->coalawebCliScripts)) return;
	...
	private function _renderPostInstallation($status, $parent)
	{
        ...
        <?php $rows = 1;?>
        <link rel="stylesheet" href="../media/com_celtawebtraffic/css/com-celtawebtraffic-base.css" type="text/css">
        <table id="newspaper-a">
            ...
            <tbody>
                <tr class="row0">
                    <td class="key" colspan="2">CeltaWeb Traffic</td>
	...
	private function _renderPostUninstallation($status, $parent) 
        ...
        <style type="text/css">
            ...
            #newspaper-a th, #newspaper-stats th {
            ...
            #newspaper-a td, #newspaper-stats td {
            ...
            <span class="cw-slider">
                <h3> CeltaWeb Traffic Uninstallation Status</h3>
            </span>
            <table id="newspaper-a">
                <thead align="left">
                    <tr>
	...
	private function _removeObsoleteFilesAndFolders($coalawebRemoveFiles)
	{
		// Remove files
		jimport('joomla.filesystem.file');
		if(!empty($coalawebRemoveFiles['files'])) foreach($coalawebRemoveFiles['files'] as $file) {
			$f = JPATH_ROOT.'/'.$file;
		...
		// Remove folders
		jimport('joomla.filesystem.file');
		if(!empty($coalawebRemoveFiles['folders'])) foreach($coalawebRemoveFiles['folders'] as $folder) {
			$f = JPATH_ROOT.'/'.$folder;
