...
# @version $Id: jsn_sampledata_helper.php 13522 2012-06-25 08:50:17Z ngocpm $
-------------------------------------------------------------------------*/
...
class JSNSampleDataHelper
{
	        ...
	        foreach ($data as $value)
			{
				...
				if ($author != '' && strtolower(trim($author)) == "joomlashine")
				{
					...
					if ($value->hasData == 'true')
					{
						...
						{
							$array_extentions[$name]['manifestPath'] = str_replace('/', DS, trim($value->manifestPath));
						}
						else
						{
	...
	function getNonBasicModule()
	{
		...
		$query 				= "SELECT " . implode(',', $field_module) . " FROM #__modules WHERE `module` NOT IN ('mod_login', 'mod_stats', 'mod_users_latest', "
	...
	function copyContentFromFilesFolder($path)
	{
		...
		if (JFolder::exists($folderPath))
		{
			...
			if ($fileList !== false)
			{
				...
				{
					if (JFile::exists($folderPath.DS.$file) && substr($file, 0, 1) != '.')
					{
						JFile::copy($folderPath.DS.$file, JPATH_ROOT.DS.$file);
					}
				}
			}
			...
			if ($folderList !== false)
			{
				...
				{
					if (JFolder::exists($folderPath.DS.$folder))
					{
						JFolder::copy($folderPath.DS.$folder, JPATH_ROOT.DS.$folder, '', true);
					}
				}
			}
	...
	function getPackageFromUpload()
	{
		...
		$tmp_dest 	= JPath::clean(JPATH_ROOT.DS.'tmp'.DS.$install_file['name']);
